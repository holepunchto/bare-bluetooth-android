...
module.exports = exports = class Peripheral extends EventEmitter {
  constructor(opts) {
    ...
    this.scanResult = opts.scanResult
  ...
  get id() {
    return this.scanResult.device.address
  }

  get name() {
    return this.scanResult.device.name
  }
  ...
  get serviceData() {
    const record = this.scanResult.scanRecord
    return record ? record.serviceData : null
  }

  get handle() {
    return this.scanResult.device._handle
  }

  _attach(peripheralHandle) {
    this._connectHandle = peripheralHandle
  ...
  destroy() {
    ...
    this._connectHandle = null
    if (this._handle) binding.peripheralDestroy(this._handle)
  }
