...
module.exports = exports = class Central extends EventEmitter {
  ...
  _ondiscover(id, name, rssi, scanRecord) {
    const device = new Device({ address: id, name })
    const record = scanRecord ? new ScanRecord(scanRecord) : null
    const scanResult = new ScanResult({ device, rssi, scanRecord: record })
