...
module.exports = exports = class Central extends EventEmitter {
  ...
  connect(peripheral) {
    binding.centralConnect(this._handle, peripheral._deviceHandle)
    this._connected.set(peripheral.id, peripheral)
  }

  disconnect(peripheral) {
    if (peripheral._handle) binding.centralDisconnect(this._handle, peripheral._handle)
  }
  ...
  _ondiscover(scanResultHandle) {
    const scanResult = new ScanResult(scanResultHandle)
    const id = scanResult.device.address
